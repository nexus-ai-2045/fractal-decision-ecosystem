...
非trivialな作業では、FDE は判断の前に team formation を設計します。
Team Creator は特定ツール名ではなく、必要な役割、分岐単位、delegate の
返却形式、採否条件、停止線 owner を作る role です。

最小 team plan は次を持ちます。
...
tiny task、tool 不在、停止線、overhead 過多などで team を作らない場合は、
`no_team_reason` を残します。
