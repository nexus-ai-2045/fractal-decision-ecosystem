...
- Linear is optional local handoff, not required for local FDE operation.
- `python scripts\mvp_gate_check.py` is the top-level private MVP gate.
- `python scripts\linear_handoff_check.py` verifies the optional Linear packet without external writes.
