...
| 短い調査 / diff要約 / 比較表 / test log整形 | `GPT-5.3-Codex-Spark` (`spark_delegate`) | `spark_candidate` を明示し、採否と正本反映だけ残す |
...
Spark / lightweight Codex は Thin Source Routing の一経路である。`thin_source_route: spark` と
`routing_decision: spark_delegate` は、差分要約・表作成・短い調査・test log整形向け。
Type1、final decision、publication approval、credential / auth / settings / destructive は
Spark に渡さず、main runtime / Team Creator が保持する。
...
FDE package lint / orchestration gate:

```bash
python3 scripts/orchestration_gate_check.py --json
python3 scripts/orchestration_gate_check.py --packet-file path/to/packet.txt
```

Orchestration smoke:

```bash
rg -n "orchestration_required|delegate_plan|codex_main_role|spark_candidate|GPT-5.3-Codex-Spark|広い読み取り|複数対象|外部AI|送信|回収|比較レビュー|packet-invalid" core.md search-orchestration.md operating-card.md
```
