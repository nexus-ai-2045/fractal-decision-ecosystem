...
手順 skill は `docs/superpowers/skills/post-merge-cleanup.md` です。CI の PR checkout では local `main` が無い前提で、`origin/main` など resolvable base ref を使い、例外は JSON fail-closed で返します。回帰は `tests/test_post_merge_cleanup.py::test_ci_checkout_without_local_main_uses_origin_main` で止めます。
