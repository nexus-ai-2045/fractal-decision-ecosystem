...
# Fractal Decision Ecosystem Axis Registry
...
| 性質 | 保存先 |
|---|---|
| 判断ルール / durable invariant | `dependency-registry:brain-general` |
| 個別 lane の決定 | `dependency-registry:lane-decisions` |
| 現在地 / 次 action | `dependency-registry:lane-status` / `dependency-registry:todo` |
| 調査・監査・実測 | `dependency-registry:reports` |
| 引き継ぎ契約 | `dependency-registry:handoff-index` |
| 判断待ち / 通信 packet | `imported-source` |
| idea / 改善案 | `dependency-registry:ideas` |
| 手順 | `dependency-registry:playbooks` |
| SSOT 管理層 (registry / loader / lint) | `dependency-registry:ssot-registry` / `ssot-loader` / `ssot-lint` |
| placement / 種別判断 / 構造分離 | `dependency-registry:typology-placement` / `placement-rules` / `structural-separation` / `ssot-split` |

## Lifecycle / Triage Axis
...
## Research / Search Axis

| source type | 例 | 主用途 |
|---|---|---|
| `library-search` | Obsidian / local docs / repo grep | 既存知識の確認 |
...
1. `local first`: repo / Documents / Obsidian / lane status / decisions を先に見る。
