...
| # | 項目 | 書くこと |
|---:|---|---|
| 1 | 目的 | 何のためにやるか。ユーザー価値を 1 行で書く |
| 2 | 主ユーザー行動 | ユーザーが実際に行う中心行動 |
| 3 | 主経路 | 最初に通す通常経路。迷ったらここへ戻る |
| 4 | 保存単位 | 何をどこへ保存し、何を保存しないか。ADR化する判断もここに書く |
| 5 | 最小E2E | 最初に通す end-to-end / smoke の 1 本 |
| 6 | やらないこと | 今回の scope から外すもの |
| 7 | fallback | 主経路が失敗した時の代替経路 |
| 8 | 公開/個人情報境界 | public に出せる情報、出せない情報、人間ゲートを書く |
| 9 | 完了条件 | 何をもって done と言えるか。未確認が残る場合は hold と書く |
| 10 | ズレ検知条件 | route drift を検知する条件 |
| 11 | owner | 判断、実装、検証、公開の責任者 |
| 12 | 検証証跡 | test、smoke、preflight、log、PR、スクリーンショットなど |
| 13 | 次の一手 | この turn で進める 1 件 |
...
吸収例（2026-07-18）: post-merge cleanup が CI で `main` 不在により例外化した。skill 文ではなく `scripts/post_merge_cleanup.py` の base-ref 解決 + fail-closed JSON + CI 形回帰テスト + `OPERATIONAL_GUARANTEE.md` へ落とした。
...
ADR は `decisions/ADR-NNNN-short-title.md` に保存し、次番号は `python scripts\adr_next.py` で採番する。カード上で「ADR化する判断」としたものは、外部 issue ではなくまず repo-local ADR として起票する。
